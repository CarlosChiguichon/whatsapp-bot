...
def create_lead(contact_name, mobile, email_from, country_id=None, segment_id=None, opportunity_mw=None):
    ...
    if segment_id:
        # Enviando segment_id como market_segment_ids pero en formato de array [6, 0, [id]]
        # que es el formato que Odoo espera para campos many2many
        payload["market_segment_ids"] = [[6, 0, [segment_id]]]
        logging.info(f"Incluyendo segmento ID: {segment_id} en el lead")
    
    if opportunity_mw:
        # Usar el nombre de campo exacto como se especificó en los requisitos
        payload["oportunity_mw"] = float(opportunity_mw)
    ...
    try:
        ...
        if response.status_code in [200, 201]:
            ...
            logging.error(f"Headers enviados: {headers}")
            logging.error(f"URL del webhook: {odoo_webhook_url}")
            
            error_message = f"Error al crear lead (código {response.status_code})"
            if response.text:
                try:
                    error_response = json.loads(response.text)
                    if isinstance(error_response, dict) and error_response.get("error"):
                        error_message = f"Error al crear lead: {error_response.get('error')}"
