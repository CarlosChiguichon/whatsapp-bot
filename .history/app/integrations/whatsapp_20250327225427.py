...
from app.integrations.openai import detect_sales_intent
...
def handle_ticket_creation(wa_id, name, message_body, session):
    ...
    # Paso 5: Procesamiento de texto para segmento
        ...
        segment_text = message_body.strip()
        segment_id = None
        
        # Mapeo por número de opción
        option_to_id = {
            "1": 1,
            "2": 2,
            "3": 3,
        ...
        # Mapeo por nombre o palabras clave
        keyword_to_id = {
            "Residencial": 1,
            "Comercial": 2,
            "Industrial": 2,
        ...
        # Primero verificar si es un número de opción
        if segment_text in option_to_id:
            segment_id = option_to_id[segment_text]
            segment_name = get_segment_name_from_id(segment_id)
            logging.info(f"Segmento seleccionado por número de opción: {segment_name} (ID: {segment_id})")
        else:
            # Si no es un número, buscar por palabras clave
            segment_text_lower = segment_text.lower()
            for key, value in keyword_to_id.items():
                if key.lower() in segment_text_lower:
                    segment_id = value
                    segment_name = get_segment_name_from_id(value)
                    logging.info(f"Segmento identificado por texto: {segment_name} (ID: {segment_id})")
                    break
        
        if segment_id:
            context['ticket_segment_id'] = segment_id
            context['ticket_segment_name'] = get_segment_name_from_id(segment_id)
            ...
            logging.warning(f"No se pudo identificar el segmento: {segment_text}")
            response = (
                "Lo siento, no pude identificar el segmento mencionado. Por favor, selecciona una de estas opciones:\n\n"
                "1) Residencial\n"
                "2) Comercial e Industrial (C&I)\n"
                "3) Utility Scale\n"
                "4) Misceláneo\n"
...
def handle_lead_creation(wa_id, name, message_body, session):
    """
    Maneja el flujo de creación de un lead comercial.
    Flujo: país -> segmento -> tamaño MW -> email -> confirmar
    ...
    """
    context = session.get('context', {})
    logging.info(f"Procesando lead creation para {name}. Paso actual: {context.get('lead_step', 'inicial')}")
    logging.info(f"Tipo de mensaje: {session.get('last_message_type', 'desconocido')}, Selection ID: {session.get('last_selection_id', 'ninguno')}")
    
    # Procesamiento de selección de país (mensaje interactivo)
    if session.get('last_message_type') == 'interactive_list' and context.get('lead_step') == 'country':
        logging.info("Procesando selección de país desde mensaje interactivo")
        selection_id = session.get('last_selection_id', '')
        country_id = get_country_id_from_selection(selection_id)
        
        if country_id == 171:  # Si es "Otro"
            context['lead_country_id'] = 171
            context['lead_country_name'] = "Otro"
            logging.info("Usuario seleccionó 'Otro' como país (ID: 171)")
            
            # Avanzar al paso de segmento
            context['lead_step'] = 'segment'
            
            # Enviar lista de selección de segmentos
            try:
                send_market_segment_list(wa_id)
                response = "Entendido. Por favor, selecciona el segmento de mercado del proyecto de la lista que aparece a continuación."
            except Exception as e:
                logging.error(f"Error al enviar lista de segmentos: {str(e)}")
                # Fallback a mensaje de texto
                send_segment_text_options(wa_id)
                response = "Entendido. Por favor, selecciona el segmento de mercado del proyecto según las opciones mostradas."
            ...
            country_name = get_country_name_from_id(country_id)
            context['lead_country_id'] = country_id
            context['lead_country_name'] = country_name
            logging.info(f"País seleccionado: {country_name} (ID: {country_id})")
            ...
            try:
                send_market_segment_list(wa_id)
                response = f"Gracias por seleccionar {country_name}. Por favor, selecciona el segmento de mercado del proyecto de la lista que aparece a continuación."
            except Exception as e:
                logging.error(f"Error al enviar lista de segmentos: {str(e)}")
                # Fallback a mensaje de texto
                send_segment_text_options(wa_id)
                response = f"Gracias por seleccionar {country_name}. Por favor, selecciona el segmento de mercado del proyecto según las opciones mostradas."
    ...
    # Procesamiento de selección de segmento (mensaje interactivo)
    if session.get('last_message_type') == 'interactive_list' and context.get('lead_step') == 'segment':
        logging.info("Procesando selección de segmento desde mensaje interactivo")
        selection_id = session.get('last_selection_id', '')
        segment_id = get_segment_id_from_selection(selection_id)
        ...
        if segment_id:
            segment_name = get_segment_name_from_id(segment_id)
            context['lead_segment_id'] = segment_id
            context['lead_segment_name'] = segment_name
            logging.info(f"Segmento seleccionado: {segment_name} (ID: {segment_id})")
            
            # Avanzar al paso de tamaño del proyecto
            context['lead_step'] = 'opportunity_mw'
            response = f"Gracias por seleccionar {segment_name}. Por favor, indica el tamaño del proyecto en MW (por ejemplo: 1.5, 2, 10). Puedes usar decimales si es necesario."
    ...
    # Paso 1: Solicitar selección de país
    if 'lead_step' not in context or context['lead_step'] == 'initial':
        logging.info(f"Iniciando creación de lead para {name}, solicitando país")
        
        # Inicializar contexto y avanzar al paso de selección de país
        context['lead_step'] = 'country'
        ...
        try:
            ...
            logging.error(f"Error al enviar lista de países: {str(e)}")
            # Fallback a mensaje de texto
            response = "Para procesar tu cotización, necesito algunos datos. Primero, indica el país donde se encuentra el proyecto (Guatemala, El Salvador, Honduras, Nicaragua, Costa Rica, Panamá, Jamaica, Barbados u Otro):"
            session_manager.update_session(wa_id, context=context)
            return response
        
        response = "Para procesar tu cotización, necesito algunos datos. Primero, selecciona el país donde se encuentra el proyecto de la lista que aparece a continuación."
    
    # Paso intermedio: si estamos en el paso country pero recibimos un mensaje de texto en lugar de selección
    elif context['lead_step'] == 'country' and session.get('last_message_type') != 'interactive_list':
        logging.info(f"Recibido texto para selección de país: {message_body}")
        # Intentar interpretar el nombre del país
        country_name = message_body.strip().title()
        country_id = None
        ...
        if country_id:
            ...
            try:
                send_market_segment_list(wa_id)
                response = f"Gracias por indicar {country_name}. Por favor, selecciona el segmento de mercado del proyecto de la lista que aparece a continuación."
            except Exception as e:
                logging.error(f"Error al enviar lista de segmentos: {str(e)}")
                # Fallback a mensaje de texto
                send_segment_text_options(wa_id)
                response = f"Gracias por indicar {country_name}. Por favor, selecciona el segmento de mercado del proyecto según las opciones mostradas."
    ...
    # Paso intermedio: si estamos en el paso segment pero recibimos un mensaje de texto en lugar de selección
    elif context['lead_step'] == 'segment' and session.get('last_message_type') != 'interactive_list':
        logging.info(f"Recibido texto para selección de segmento: {message_body}")
        segment_text = message_body.strip()
        segment_id = None
        ...
        if segment_id:
            context['lead_segment_id'] = segment_id
            context['lead_segment_name'] = get_segment_name_from_id(segment_id)
            
            # Avanzar al paso de tamaño del proyecto
            context['lead_step'] = 'opportunity_mw'
            response = f"Gracias por seleccionar {context['lead_segment_name']}. Por favor, indica el tamaño del proyecto en MW (por ejemplo: 1.5, 2, 10). Puedes usar decimales si es necesario."
    ...
    # Paso 3: Recopilar el tamaño del proyecto en MW
    elif context['lead_step'] == 'opportunity_mw':
        logging.info(f"Recopilando tamaño del proyecto para {name}: {message_body}")
        
        # Validar formato de número (entero o decimal)
        try:
            # Limpiar el mensaje para el procesamiento
            # Reemplazar comas por puntos y eliminar cualquier texto no numérico
            clean_message = message_body.replace(',', '.')
            # Extraer el primer número que encontremos (entero o decimal)
            import re
            ...
            opportunity_mw = float(number_match.group(1))
            
            if opportunity_mw <= 0:
                response = "El tamaño del proyecto debe ser un número positivo. Por favor, ingresa un valor válido en MW."
                return response
                
            context['lead_opportunity_mw'] = opportunity_mw
            logging.info(f"Tamaño del proyecto: {opportunity_mw} MW")
            
            # Avanzar al paso de correo electrónico
            context['lead_step'] = 'email'
            response = "Gracias por la información. Por favor, proporciona tu correo electrónico para enviarte la cotización. Es un dato necesario para procesar tu solicitud."
            
            session_manager.update_session(wa_id, context=context)
            return response
            
        except ValueError:
            logging.warning(f"Formato inválido para tamaño de proyecto: {message_body}")
            response = "Por favor, ingresa el tamaño del proyecto como un número (por ejemplo: 1.5, 2, 10). Usa solo números y un punto decimal si es necesario."
            return response
    
    # Paso 4: Solicitar email (requerido)
    elif context['lead_step'] == 'email':
        # Validar formato básico de email
        email_pattern = r'^[\w\.-]+@[\w\.-]+\.\w+$'
        if re.match(email_pattern, message_body):
            context['lead_email'] = message_body
            logging.info(f"Correo electrónico proporcionado por {name}: {message_body}")
            
            # Avanzar al paso de confirmación
            context['lead_step'] = 'confirmation'
            
            # Mostrar resumen y pedir confirmación
            country_info = f"*País:* {context.get('lead_country_name', 'No especificado')}"
            segment_info = f"*Segmento:* {context.get('lead_segment_name', 'No especificado')}"
            mw_info = f"*Tamaño del proyecto:* {context.get('lead_opportunity_mw', 'No especificado')} MW"
            email_info = f"*Email:* {context['lead_email']}"
            
            response = (
                "Por favor, confirma los detalles de tu solicitud de cotización:\n\n"
                f"{country_info}\n"
                f"{segment_info}\n"
                f"{mw_info}\n"
                f"{email_info}\n\n"
                "¿Deseas enviar esta solicitud con esta información? (responde 'sí' o 'no')"
            )
    ...
    # Paso 5: Confirmar y crear el lead
    elif context['lead_step'] == 'confirmation':
        if message_body.lower() in ['si', 'sí', 'yes', 'confirmar', 'aceptar', 'ok']:
            # Mostrar mensaje de espera
            logging.info(f"Iniciando creación de lead para {name} ({wa_id})")
            
            # Crear el lead en Odoo
            from app.integrations.odoo import create_lead
            
            # Preparar datos para el lead
            lead_data = {
                "contact_name": name,
                "mobile": wa_id,
                "email_from": context['lead_email'],
            }
            
            # Añadir ID de país si está disponible
            if 'lead_country_id' in context:
                lead_data["country_id"] = context['lead_country_id']
                logging.info(f"Añadiendo country_id: {context['lead_country_id']} al lead")
            
            # Añadir segmento de mercado si está disponible
            if 'lead_segment_id' in context:
                lead_data["segment_id"] = context['lead_segment_id']
                logging.info(f"Añadiendo segment_id: {context['lead_segment_id']} al lead")
            
            # Añadir tamaño del proyecto si está disponible
            if 'lead_opportunity_mw' in context:
                lead_data["opportunity_mw"] = context['lead_opportunity_mw']
                logging.info(f"Añadiendo opportunity_mw: {context['lead_opportunity_mw']} al lead")
            
            # Asegurar que todos los valores enviados son los correctos
            logging.info(f"Datos de lead a enviar: {lead_data}")
            
            # Crear el lead
            lead_result = create_lead(**lead_data)
            
            logging.info(f"Resultado de creación de lead: {lead_result}")
            
            if lead_result.get('success', False):
                lead_id = "N/A"
                if isinstance(lead_result.get('data'), dict) and 'id' in lead_result.get('data', {}):
                    lead_id = lead_result['data']['id']
                
                response = (
                    f"¡Solicitud de cotización #{lead_id} enviada con éxito! Un miembro de nuestro equipo comercial revisará tu solicitud y te contactará pronto a través del correo electrónico proporcionado. "
                    "¿Necesitas ayuda con algo más? (responde 'sí' o 'no')"
                )
                # Cambiar a estado de espera post-lead
                session_manager.update_session(wa_id, state='AWAITING_RESPONSE_POST_LEAD', context={})
            else:
                error_msg = lead_result.get('error', 'Error desconocido')
                error_details = ""
                
                # Personalizar mensaje según el tipo de error
                if "webhook" in error_msg.lower() or "URL" in error_msg:
                    error_details = "No se pudo contactar al servidor de cotizaciones."
                elif "timeout" in error_msg.lower():
                    error_details = "El servidor de cotizaciones tardó demasiado en responder."
                elif "500" in error_msg:
                    error_details = "Hubo un problema interno en el servidor de cotizaciones."
                
                response = (
                    f"Lo siento, hubo un problema al enviar tu solicitud de cotización: {error_msg}. {error_details} "
                    "Hemos registrado este error y lo revisaremos pronto. "
                    "Por favor, intenta de nuevo más tarde o contacta directamente con nuestro equipo comercial."
                )
                # Restablecer el estado para nuevas consultas
                session_manager.update_session(wa_id, state='AWAITING_QUERY', context={})
                
                # Registrar error detallado
                logging.error(f"Error al crear lead para {name} ({wa_id}): {lead_result}")
        else:
            response = "Solicitud de cotización cancelada. ¿En qué más puedo ayudarte?"
            # Restablecer el estado para nuevas consultas
            session_manager.update_session(wa_id, state='AWAITING_QUERY', context={})
    
    else:
        # Si por alguna razón el paso no está definido correctamente
        logging.error(f"Paso de lead indefinido para {name}: {context.get('lead_step', 'None')}")
        response = "Lo siento, hubo un problema con el proceso de solicitud de cotización. ¿Puedes intentarlo de nuevo?"
        session_manager.update_session(wa_id, state='AWAITING_QUERY', context={})
    
    # Actualizar el contexto si seguimos en creación de lead
    if session['state'] == 'LEAD_CREATION':
        session_manager.update_session(wa_id, context=context)
...
def process_message(message_data):
    ...
    if message_type == "text":
        ...
        elif session['state'] == 'AWAITING_RESPONSE_POST_LEAD':
            # Estado después de crear lead
            if message_body.lower() in ['no', 'nop', 'nope', 'negativo', 'n']:
                close_session_with_message(wa_id, name)
                return
            else:
        ...
        elif session['state'] == 'LEAD_CREATION':
            # Proceso de creación de lead
            response = handle_lead_creation(wa_id, name, message_body, session)
        ...
        # Detectar intención de cotización o compra
        elif detect_sales_intent(message_body) and session['state'] not in ['TICKET_CREATION', 'LEAD_CREATION']:
            response = "Gracias por tu interés en nuestros productos y servicios. Me gustaría recopilar algunos datos para enviarte una cotización personalizada."
            session_manager.update_session(
                wa_id, 
                state='LEAD_CREATION', 
                context={'lead_step': 'initial'},
                last_message_type='text'
            )
            
            # Iniciar inmediatamente el flujo de creación de lead enviando la lista de países
            try:
                send_country_selection_list(wa_id)
                logging.info(f"Lista de países enviada a {name} al iniciar lead")
                response += " Para comenzar, por favor selecciona el país donde se encuentra el proyecto de la lista que aparece a continuación."
            except Exception as e:
                logging.error(f"Error al enviar lista de países inicial: {str(e)}")
                response += " Para comenzar, por favor indica el país donde se encuentra el proyecto."
    ...
    # Manejar respuestas de listas interactivas (selección de país)
        ...
        # Verificar si estamos en proceso de creación de ticket o lead
        ticket_creation = session['state'] == 'TICKET_CREATION'
        lead_creation = session['state'] == 'LEAD_CREATION'
        
        ticket_country_step = session.get('context', {}).get('ticket_step') == 'country'
        ticket_segment_step = session.get('context', {}).get('ticket_step') == 'segment'
        
        lead_country_step = session.get('context', {}).get('lead_step') == 'country'
        lead_segment_step = session.get('context', {}).get('lead_step') == 'segment'
        
        logging.info(f"¿Estamos en creación de ticket? {ticket_creation}")
        logging.info(f"¿Estamos en creación de lead? {lead_creation}")
        
        # Si estamos en proceso de creación de ticket y en el paso correspondiente
        if ticket_creation and (ticket_country_step or ticket_segment_step):
            # Procesar la selección para ticket
            response = handle_ticket_creation(wa_id, name, selection_text, session)
        # Si estamos en proceso de creación de lead y en el paso correspondiente
        elif lead_creation and (lead_country_step or lead_segment_step):
            # Procesar la selección para lead
            response = handle_lead_creation(wa_id, name, selection_text, session)
        else:
            # Si no estamos en el flujo esperado, proporcionar una respuesta general
            logging.warning(f"Recibida selección interactiva fuera del flujo esperado: {selection_text}. Estado: {session['state']}")
            
            # Verificar si la selección es un país y forzar el inicio del proceso adecuado
            if selection_id.startswith('country_'):
                logging.info(f"Forzando inicio de proceso con selección de país: {selection_text}")
                
                # Determinar si es más probable un ticket o un lead
                if detect_ticket_intent(selection_text):
                    ...
                    # Establecer estado de creación de lead por defecto
                    session_manager.update_session(
                        wa_id,
                        state='LEAD_CREATION',
                        context={'lead_step': 'country'},
                    ...
                    # Obtener sesión actualizada y procesar
                    updated_session = session_manager.get_session(wa_id)
                    response = handle_lead_creation(wa_id, name, selection_text, updated_session)
                    
            # Verificar si la selección es un segmento y forzar el proceso adecuado
            elif selection_id.startswith('segment_'):
                logging.info(f"Forzando procesamiento de selección de segmento: {selection_text}")
                
                # Determinar si es más probable un ticket o un lead
                if 'ticket' in selection_text.lower() or session.get('context', {}).get('ticket_step'):
                    # Establecer estado y paso apropiados para ticket
                    session_manager.update_session(
                        wa_id,
                        state='TICKET_CREATION',
                        context={'ticket_step': 'segment'},
                    ...
                    # Establecer estado y paso apropiados para lead
                    session_manager.update_session(
                        wa_id,
                        state='LEAD_CREATION',
                        context={'lead_step': 'segment'},
